	...
	public class SaveFileHandler_SystemFile : iSaveFileHandler
	{
		...
		protected SaveFile GetSaveFile (int saveID, int profileID, bool isImport, int boolID, string separateProductName, string separateFilePrefix)
		{
			...
			if (File.Exists (fullFilename))
			{
				...
				string label = ((isImport) ? "Import " : "Save ") + saveID.ToString ();
				if (saveID == 0)
				{
					label = "Autosave";
					isAutoSave = true;
				...
				return new SaveFile (saveID, profileID, label, fullFilename, isAutoSave, screenShot, screenshotFilename, updateTime);
			}
