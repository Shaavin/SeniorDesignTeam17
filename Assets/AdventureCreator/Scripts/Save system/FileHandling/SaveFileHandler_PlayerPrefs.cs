	...
	public class SaveFileHandler_PlayerPrefs : iSaveFileHandler
	{
		...
		protected SaveFile GetSaveFile (int saveID, int profileID, bool isImport, int boolID, string separateFilePrefix)
		{
			...
			if (PlayerPrefs.HasKey (filename))
			{
				...
				if (KickStarter.settingsManager.takeSaveScreenshots && PlayerPrefs.HasKey (filename + screenshotKey) && KickStarter.saveSystem)
				{
					try
					{
				...
				return new SaveFile (saveID, profileID, label, filename, isAutoSave, screenShot, string.Empty, updateTime);
			}
