		...
		protected IEnumerator UseObject (InvInstance selectedInvInstance)
		{
			...
			if (KickStarter.player)
			{
				if (button != null && button.playerAction != PlayerAction.DoNothing)
				{
					...
					{
						Vector3 _hotspotCentre = (_hotspot.centrePoint) ? _hotspot.centrePoint.position : _hotspot.Transform.position;
		...
		public int GetActiveUseButtonIconID ()
		{
			if (KickStarter.settingsManager.SelectInteractionMethod () == SelectInteractions.CyclingCursorAndClickingHotspot)
			{
				if (InvInstance.IsValid (KickStarter.runtimeInventory.HoverInstance) && KickStarter.settingsManager.InventoryInteractions == AC.InventoryInteractions.Multiple)
				{
					...
					{
						if (KickStarter.runtimeInventory.HoverInstance.InvItem.interactions == null || KickStarter.runtimeInventory.HoverInstance.InvItem.interactions.Count == 0)
						{
							return -1;
						}
						else
					...
					if (KickStarter.runtimeInventory.HoverInstance.InvItem.interactions != null && interactionIndex < KickStarter.runtimeInventory.HoverInstance.InvItem.interactions.Count)
					{
						return KickStarter.runtimeInventory.HoverInstance.InvItem.interactions [interactionIndex].icon.id;
					}
				}
