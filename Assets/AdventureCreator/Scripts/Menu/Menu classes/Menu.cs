...
namespace AC
{
	...
	public class Menu : ScriptableObject
	{
		...
		public void ShowGUI ()
		{
			...
			if (KickStarter.settingsManager && KickStarter.settingsManager.takeSaveScreenshots)
			{
		...
		{
			if (useAspectRatio && KickStarter.settingsManager && !KickStarter.settingsManager.forceAspectRatio)
			{
		...
		public void MatchInteractions (InvInstance invInstance, bool includeInventory)
		{
			...
			foreach (MenuElement element in elements)
			{
				...
				{
					MenuInteraction interaction = (MenuInteraction) element;
					interaction.MatchInteractions (invInstance.InvItem);
				}
				else if (element is MenuInventoryBox)
				{
					if (includeInventory)
		...
		/**
		 * <summary>Assigns the Menu, and all MenuElement classes within it, to a Hotspot or inventory item.</summary>
		 */
