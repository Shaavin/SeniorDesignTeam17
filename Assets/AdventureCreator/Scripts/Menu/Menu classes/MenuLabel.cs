		...
		public void UpdateLabelText (int languageNumber = 0)
		{
			...
			switch (labelType)
			{
				...
				case AC_LabelType.InventoryProperty:
					...
					{
						newLabel = GetPropertyDisplayValue (languageNumber, KickStarter.runtimeInventory.SelectedInstance);
					}
					else if (inventoryPropertyType == InventoryPropertyType.LastClickedItem)
					{
						newLabel = GetPropertyDisplayValue (languageNumber, KickStarter.runtimeInventory.LastClickedInstance);
					}
					else if (inventoryPropertyType == InventoryPropertyType.MouseOverItem)
					{
						newLabel = GetPropertyDisplayValue (languageNumber, KickStarter.runtimeInventory.HoverInstance);
					}
					break;
