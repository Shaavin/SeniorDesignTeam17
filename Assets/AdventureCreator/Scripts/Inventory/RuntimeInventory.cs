...
namespace AC
{
	...
	public class RuntimeInventory : MonoBehaviour
	{
		 ...
		 */
		/*public void Combine (InvInstance item1, int item2ID)
		{
				...
				case AC_InventoryBoxType.DisplayLastSelected:
					{
						if (KickStarter.settingsManager.InventoryInteractions == InventoryInteractions.Multiple && KickStarter.playerMenus.IsInteractionMenuOn ())
						{
							...
							ClickInvItemToInteract (inventoryBox.GetInstance (_slot));
							clickConsumed = true;
						}
						else if (KickStarter.settingsManager.InventoryInteractions == InventoryInteractions.Multiple && KickStarter.settingsManager.SelectInteractionMethod () == SelectInteractions.CyclingCursorAndClickingHotspot)
						{
							...
							{
								clickConsumed = inventoryBox.HandleDefaultClick (_mouseState, _slot, KickStarter.settingsManager.interactionMethod);
							}
						...
						else if (KickStarter.settingsManager.InventoryInteractions == InventoryInteractions.Single)
						{
							clickConsumed = inventoryBox.HandleDefaultClick (_mouseState, _slot, AC_InteractionMethod.ContextSensitive);
						}
						else
						{
							clickConsumed = inventoryBox.HandleDefaultClick (_mouseState, _slot, KickStarter.settingsManager.interactionMethod);
