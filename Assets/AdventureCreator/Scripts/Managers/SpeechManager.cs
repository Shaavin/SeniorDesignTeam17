	...
	public class SpeechManager : ScriptableObject
	{
		...
		public void ShowGUI (Rect position)
		{
			...
			if (showSubtitles)
			{
				...
				waitTimeOffset = CustomGUILayout.Slider ("Default post-line delay (s):", waitTimeOffset, 0f, 4f, "AC.KickStarter.speechManager.waitTimeOffset", "The default delay to insert after triggering a line of dialogue with the 'Dialogue: Play speech' Action. A further delay can be added per-Action.");
		...
		private void CollectAllActionListAssets ()
		{
			...
			foreach (string sceneFile in sceneFiles)
			{
				...
				// Dialogue options
				Conversation[] conversations = FindObjectsOfType (typeof (Conversation)) as Conversation[];
				foreach (Conversation conversation in conversations)
				{
				...
				SetParametersBase[] setParameterBases = FindObjectsOfType (typeof (SetParametersBase)) as SetParametersBase[];
				...
				// Draggable
				Moveable_Drag[] moveable_Drags = FindObjectsOfType (typeof (Moveable_Drag)) as Moveable_Drag[];
				...
				// PickUp
				Moveable_PickUp[] moveable_PickUps = FindObjectsOfType (typeof (Moveable_PickUp)) as Moveable_PickUp[];
