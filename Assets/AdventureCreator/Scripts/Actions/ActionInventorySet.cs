			...
			if (KickStarter.runtimeInventory)
			{
				...
				switch (invAction)
				{
					case InvAction.Add:
						...
						{
							KickStarter.runtimeInventory.AddToOtherPlayer (new InvInstance (invID, amount), _playerID, addToFront);
						}
						else
						{
							if (addToFront)
							{
								KickStarter.runtimeInventory.PlayerInvCollection.Insert (new InvInstance (invID, amount), 0);
							}
							else
							{
								KickStarter.runtimeInventory.PlayerInvCollection.Add (new InvInstance (invID, amount));
							}
						}
						break;
